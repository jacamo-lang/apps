...
is_wait_repair_goal :- is_disabled & position(MyV) & myTeam(MyTeam) & 
						edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, normal) & friend(_, Entity, repairer).
...
is_wait_repair_goal :- is_disabled & position(MyV) & myTeam(MyTeam) & .my_name(MyAgentName) & 
						edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, _) & friend(AgentName, Entity, repairer) & priorityEntity(AgentName, MyAgentName).
