...
/* GOTO disabled state */
+!goto(Op):
	is_disabled & maxEnergy(MaxE) & position(MyV) & edge(MyV, Op, W) & infinite(W) & 10 > MaxE //I have to buy more batteries and I don't have enough money
<-
...
+!goto(Op):
	is_disabled & energy(MyE) & position(MyV) & edge(MyV, Op, W) & infinite(W) & 10 > MyE //That's too expensive
<-
	.print("I'm disabled. My current energy is ", MyE, " but I need ", 10);
	!recharge. //firstly, I'm going to recharge.
	
+!goto(Op):
	is_disabled & position(MyV) & edge(MyV, Op, W) & infinite(W) //I can go!
<-
...
<-
	.print("ENABLED My max energy is ", MaxE, " but I need ", W);
...
+!broadcastVisit(V, S):
	not visitedVertex(V, S)
<-
	.broadcast(tell,visitedVertex(V, S)).
+!broadcastVisit(V, S): true <- true.


+visitedVertex(V, S) [source(self)]: true <- true.
@visitedVertex1[atomic]
+visitedVertex(V, S):
	true
<-
	.abolish(visitedVertex(V, _));
...
+surveyedEdge(U, V, W) [source(percept)]: 
	infinite(INF) & edge(U, V, INF) 
<-
	//!broadcastSurvey(V, U, W);
	-edge(U, V, _); -edge(V, U, _);
	+edge(U, V, W); +edge(V, U, W);
	ia.addEdge(U, V, W).

+edge(U, V, W) [source(self)]: true <- true.
@surveyedEdge2[atomic]
+edge(U, V, W):
	infinite(INF) & edge(U, V, INF) 
<-
	-edge(U, V, _); -edge(V, U, _);
...
+!broadcastSurvey(U, V, W):
	infinite(INF) & edge(U, V, INF)
<-
	.broadcast(tell,edge(U, V, W)).
+!broadcastSurvey(U, V, W): true <- true.
