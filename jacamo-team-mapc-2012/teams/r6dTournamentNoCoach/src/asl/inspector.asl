...
//+!select_goal : not is_almost_full_map & not is_disabled & get_vertex_to_go_inspect_search(D, Path) <- !init_goal(gotoPathInspect(Path)).
...
+!gotoPathSurvey([]):
	true
<-
	//.print("I'm mode inspect on. I do not know any path and I'm lost.");
	!random_walk.
	
+!gotoPathInspect([V|[]]):
	position(V)
<-
	//.print("I'm mode inspect on. I do not know any path and I'm lost two.");
	!random_walk.
...
+!gotoPathInspect([V|_]):
	true
<-
	//.print("I'm mode inspect on. My next step is ", V);
	!goto(V).
