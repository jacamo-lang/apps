...
+!notifySaboteursAboutEnemy(Entity, V, Team, normal):
	...
	.my_name(saboteur1)
<-
	.send(saboteur2, tell, visibleEntityTell(Entity, Team, V, normal));
	.send(saboteur3, tell, visibleEntityTell(Entity, Team, V, normal));
	.send(saboteur4, tell, visibleEntityTell(Entity, Team, V, normal)).
	
+!notifySaboteursAboutEnemy(Entity, V, Team, normal):
	is_dangerous_entity(Entity) & 
	.my_name(saboteur2)
<-
	.send(saboteur1, tell, visibleEntityTell(Entity, Team, V, normal));
	.send(saboteur2, tell, visibleEntityTell(Entity, Team, V, normal));
	.send(saboteur4, tell, visibleEntityTell(Entity, Team, V, normal)).
...
+!unnotifySaboteursAboutEnemy(Entity, V, Team, normal):
	...
	.my_name(saboteur1)
<-
	.send(saboteur2, untell, visibleEntityTell(Entity, Team, V, _));
	.send(saboteur3, untell, visibleEntityTell(Entity, Team, V, _));
	.send(saboteur4, untell, visibleEntityTell(Entity, Team, V, _)).
	
+!unnotifySaboteursAboutEnemy(Entity, V, Team, normal):
	is_dangerous_entity(Entity) &
	.my_name(saboteur2)
<-
	.send(saboteur1, untell, visibleEntityTell(Entity, Team, V, _));
	.send(saboteur3, untell, visibleEntityTell(Entity, Team, V, _));
	.send(saboteur4, untell, visibleEntityTell(Entity, Team, V, _)).
