 ...
 * I have enough money
 */
is_buy_goal    :- not limitBuy & (is_buy_attack_goal | is_buy_shield_goal) & money(MyM) & MyM > 2.
				 
is_buy_attack_goal    :- 
	strength(MyS) &
	(MyS < 6 | strengthRequired(Sr) & MyS < Sr) & .my_name(saboteur4).
...
is_buy_shield_goal    :- 
		...
		myMaxHealth(MyH) & ( 
			MyH < 4
		|
			healthRequired(Hr) & MyH < Hr & 
			.my_name(saboteur4)
		|
			healthRequired(Hr) & MyH < Hr & step(S) & 
			//lastBuy(LastS) & S - LastS >= 50 &
			lastDie(LastDie) & S - LastDie <= 50
		).
...
+!select_goal: not strategy(_) <- +strategy(1); !select_goal.
+!select_goal:
...
/*+!select_goal : there_is_enemy_nearby(Op) & there_is_dangerous_enemy_at(Op) &
				is_full_energy_goal & 
				not is_disabled <- !init_goal(recharge). */
...
+!select_goal : not .my_name(saboteur4) & there_is_enemy_inside_my_good_area(V) & get_vertex_to_go_attack(D, Path) <- !init_goal(gotoPathAttack(Path)).
+!select_goal : (strategy(2) | strategy(3)) & get_vertex_to_go_attack_search(D, Path) & not is_at_swarm_position_test <- !init_goal(gotoPathAttack(Path)).
+!select_goal : strategy(1) & .my_name(saboteur4) & get_vertex_to_go_attack_search(D, Path) <- !init_goal(gotoPathAttack(Path)).
+!select_goal : strategy(1) & .my_name(saboteur4) & get_vertex_getting_around(D, Path) <-
	.print("Getting around to ", D); 
	!init_goal(gotoPathAttack(Path)).


+!select_goal: strategy(1) & .my_name(saboteur4) & noMoreVertexToProbe & not is_at_swarm_position & get_vertex_to_go_swarm(D, Path) <-
	!clearGettingAround;
...
+!select_goal: strategy(1) & .my_name(saboteur4) <-
	!clearGettingAround; 
...
+!newPositionGettingAround(V):
	get_all_best_vertex(List) & .member(V, List) & not visitedBestVertex(V)
<-
	.print("Getting around arrived at ", V);
	+visitedBestVertex(V).
