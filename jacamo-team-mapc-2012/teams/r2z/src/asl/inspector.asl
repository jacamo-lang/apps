...
is_inspect_goal :- position(MyV) & myTeam(MyTeam) & visibleEntity(Entity, MyV, Team, _) & Team \== MyTeam 
				  & need_inspect(Entity) & not there_is_dangerous_enemy_at(MyV) &
				  i_have_more_priority.
				  
need_inspect(Entity) :- timeBetweenInspect(Time) & step(S) & 
						not entityType(Entity, _) | entityType(Entity, "Saboteur") & lastInspectEntity(Entity, SInspect) & S - SInspect > Time.
						
						
is_inspect_goal_swarm :- position(MyV) & myTeam(MyTeam) & visibleEntity(Entity, V, Team, _) & Team \== MyTeam & ia.edge(MyV,V,_) 
				  & need_inspect_swarm(Entity).
...
need_inspect_swarm(Entity) :- timeBetweenInspect(Time) & step(S) & 
						not entityType(Entity, _) | lastInspectEntity(Entity, SInspect) & S - SInspect > Time.
...
+!select_goal : is_leave_goal & not is_almost_full_map  <- !init_goal(random_walk).
...
<-
	...
	!!broadcastInspectForSaboteurs(Entity, MaxHealth);
...
<-
	...
	!broadcastInspectForSaboteursAndRepairers(Strength).
+!evaluateStrength(Strength, Type): true <- true.
	
+!broadcastInspectForSaboteursAndRepairers(MaxStrength):
	true
<-
	.send(saboteur1, achieve, setEntityNewMaxStrength(MaxStrength));
	.send(saboteur2, achieve, setEntityNewMaxStrength(MaxStrength));
	.send(saboteur3, achieve, setEntityNewMaxStrength(MaxStrength));
	.send(saboteur4, achieve, setEntityNewMaxStrength(MaxStrength)).
	
	//.send(repairer1, achieve, setEntityNewMaxStrength(MaxStrength));
	//.send(repairer2, achieve, setEntityNewMaxStrength(MaxStrength));
	//.send(repairer3, achieve, setEntityNewMaxStrength(MaxStrength));
	//.send(repairer4, achieve, setEntityNewMaxStrength(MaxStrength)).
