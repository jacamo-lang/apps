...
there_is_enemy_inside_my_good_area(V) :- there_is_enemy_at(V) & 
...
there_is_enemy_inside_my_good_area(V) :- 
									  (myTeam(MyTeam) & (visibleEntity(Entity, Op, Team, normal) | visibleEntityTell(Entity, Op, Team, normal)) & Team \== MyTeam & (entityType(Entity, "Saboteur") | not entityType(Entity, _))) & 
									  allowedArea(NeighborhoodOutside, NeighborhoodInside) &
									  (.member(V, NeighborhoodOutside) | .member(V, NeighborhoodInside)).
									  
is_dangerous_entity(Entity) :- myTeam(MyTeam) & entity(Entity, Team, _, _) & Team \== MyTeam & (not entityType(Entity, _) | entityType(Entity, "Saboteur")).
