...
+!notifySaboteursAboutEnemy(Entity, V, Team, normal):
	is_dangerous_entity(Entity) & 
	.my_name(saboteur1)
<-
	.send(saboteur2, tell, visibleEntity(Entity, Team, V, normal));
	.send(saboteur3, tell, visibleEntity(Entity, Team, V, normal));
	.send(saboteur4, tell, visibleEntity(Entity, Team, V, normal)).
	
+!notifySaboteursAboutEnemy(Entity, V, Team, normal):
	is_dangerous_entity(Entity) & 
	.my_name(saboteur2)
<-
	.send(saboteur1, tell, visibleEntity(Entity, Team, V, normal));
	.send(saboteur2, tell, visibleEntity(Entity, Team, V, normal));
	.send(saboteur4, tell, visibleEntity(Entity, Team, V, normal)).
...
+!unnotifySaboteursAboutEnemy(Entity, V, Team, normal):
	...
	.my_name(saboteur1)
<-
	.send(saboteur2, untell, visibleEntity(Entity, Team, V, _));
	.send(saboteur3, untell, visibleEntity(Entity, Team, V, _));
	.send(saboteur4, untell, visibleEntity(Entity, Team, V, _)).
	
+!unnotifySaboteursAboutEnemy(Entity, V, Team, normal):
	is_dangerous_entity(Entity) &
	.my_name(saboteur2)
<-
	.send(saboteur1, untell, visibleEntity(Entity, Team, V, _));
	.send(saboteur3, untell, visibleEntity(Entity, Team, V, _));
	.send(saboteur4, untell, visibleEntity(Entity, Team, V, _)).
...
+!setEntityNewMaxStrength(MaxStrength):
	healthRequired(Current) & MaxStrength > Current -1
<-
	-+healthRequired(MaxStrength +1).
+!setEntityNewMaxStrength(MaxStrength): true <- true.	