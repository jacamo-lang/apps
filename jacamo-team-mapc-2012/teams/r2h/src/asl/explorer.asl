...
/*
+!tryProposeBestCoverage(S):	
	lastCalcCoverage(N) & S - N >= 15 & .my_name(explorer1) & S < 130
<- 
...
+!tryProposeBestCoverage(S):	
	lastCalcCoverage(N) & S - N >= 15 & .my_name(AgentName) & S < 130
<- 
	-+lastCalcCoverage(S);
	.print("COVERAGE Calculing the best coverage...");
	ia.bestCoverage(1, BestVertex, BestValue);
...
<-
	.print("COVERAGE ALL Proposal arrived!");
	!decideBestCoverageProposal.
	
+!decideBestCoverageProposal:
	get_best_coverage_proposal(BestVertex, BestValue, Neighborhood)
<-
	.broadcast(achieve, setNewBestCoverageSwarm(BestVertex, BestValue, Neighborhood));
	.abolish(bestCoverageProposal(_, _, _, _));
	.print("COVERAGE DECISION RESULT: BestVertex: ", BestVertex, " BestValue: ", BestValue, " Neighborhood: ", Neighborhood);
