...
is_leave_goal :- position(MyV) & myTeam(MyTeam) & visibleEntity(Entity, MyV, Team, normal) & Team \== MyTeam &
...
there_is_enemy_at(Op) :- myTeam(MyTeam) & 
						visibleEntity(Entity, Op, Team, normal) & 
						Team \== MyTeam & is_dangerous_entity(Entity).
						
there_is_any_enemy_at(Op) :- myTeam(MyTeam) & 
						visibleEntity(Entity, Op, Team, normal) & 
						Team \== MyTeam.
						
there_is_enemy_neighborhood(U) :- ia.edge(U,V,_) & there_is_enemy_at(V).
there_is_dangerous_enemy_at(Op) :- myTeam(MyTeam) & 
								   visibleEntity(Entity, Op, Team, normal) & 
								   Team \== MyTeam & 
								   (entityType(Entity, "Saboteur") | is_suspect(Entity)).
...
there_is_enemy_inside_my_good_area(V) :- 
									  (
									  	(visibleEntity(Entity, V, Team, normal) | ia.visibleEnemy(Entity, V)) & 
									  	not friend(_, Entity, _) & 
									  	(   
									  		entityType(Entity, "Saboteur") 
									  		| 
									  		not entityType(Entity, _) & .count((entityType(_, "Saboteur")), N) & N < 4
									  		| 
									  		is_suspect(Entity)
									  	)
									  ) & 
...
//Test also other kind of agents
there_is_enemy_inside_my_good_area(V) :- 
									  (
									  	(visibleEntity(Entity, V, Team, normal) | ia.visibleEnemy(Entity, V)) & 
									  	not friend(_, Entity, _)
...
is_dangerous_entity(Entity) :- myTeam(MyTeam) & 
...
is_suspect(Entity) :- suspect(Entity) & 
						not entityType(Entity, _) & 
						.count((entityType(_, "Saboteur")), N) & N < 4.
