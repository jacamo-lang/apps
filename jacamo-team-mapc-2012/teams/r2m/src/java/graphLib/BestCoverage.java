...
public class BestCoverage {
	...
	public ArrayList<Integer> execute(Graph g, int d) {
		...
		if (best != Graph.NULL) {
			...
			System.out.println("Melhor vertice ONE: " + best + " Value: " + g.values[best] + " Valor total: " + vBest);
		}
		return list;
	}
		...
		for (int i = 0; i <= g.getSize(); i++) {
			...
			//if (alive[i]) {
				...
				for (int j = 0; j < g.grade[u]; j++) {
					int v = g.adj[u][j];
					if (white[v] && g.values[u] == g.values[v]) {
						if (neighborhoodIgnore.contains(v)) {
							nIgnores++;
						}
						white[v] = false;
						depth[v] = 0;
				...
				if (nIgnores == neighborhoodIgnore.size()) {
					//System.out.println("Ignoring vertex " + i + " because " + nIgnores + " = " + neighborhoodIgnore.size());
					continue;
				}
