...
is_inside_team_region :- not (position(MyV) & myTeam(MyTeam) & ia.edge(MyV,V,_) & ia.vertex(V, Team) & MyTeam \== Team).
...
is_linked_region_weak :- position(MyV) & myTeam(MyTeam) &
					.setof(V, 
						   ia.edge(MyV,V,_) & ia.vertex(V, MyTeam) & visibleEntity(_, V, MyTeam, normal), 
						   Options
					) & .length(Options, TotalVertexMyTeam) & TotalVertexMyTeam >= 2.
