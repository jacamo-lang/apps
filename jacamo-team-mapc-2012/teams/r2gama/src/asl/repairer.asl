...
{ include("commonRules_notRepairer.asl") } //YES!
...
/*
 * Check if there is a friend at some adjacent vertex and he is disabled and I'm not disabled
 */
there_is_disabled_repairer_friend_nearby(Op) :- not is_disabled & position(MyV) & myTeam(MyTeam) & maxWeight(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & friend(_, Entity, repairer) & not there_is_enemy_at(V), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).
						   
there_is_disabled_repairer_friend_nearby(Op) :- not is_disabled &  position(MyV) & myTeam(MyTeam) & infinite(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & friend(_, Entity, repairer) & not there_is_enemy_at(V), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).

//Now I'm disabled
there_is_disabled_repairer_friend_nearby(Op) :- is_disabled &  position(MyV) & myTeam(MyTeam) & maxWeight(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & friend(_, Entity, repairer) & there_is_disable_friend_at_same_vertex_as_repairer(V) & not there_is_enemy_at(V), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).
						   
there_is_disabled_repairer_friend_nearby(Op) :- is_disabled &  position(MyV) & myTeam(MyTeam) & infinite(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & friend(_, Entity, repairer) & there_is_disable_friend_at_same_vertex_as_repairer(V) & not there_is_enemy_at(V), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).

//Other friends not repairers
there_is_disabled_friend_nearby(Op) :- position(MyV) & myTeam(MyTeam) & maxWeight(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & not there_is_enemy_at(V) & friend(_, Entity, saboteur), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).
there_is_disabled_friend_nearby(Op) :- position(MyV) & myTeam(MyTeam) & infinite(INF) & myNameInContest(MyName) &
						   ...
						   .nth(math.random(TotalOptions), Options, Op).
there_is_disabled_friend_nearby(Op) :- position(MyV) & myTeam(MyTeam) & maxWeight(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & not there_is_enemy_at(V) & friend(_, Entity, explorer), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).
there_is_disabled_friend_nearby(Op) :- position(MyV) & myTeam(MyTeam) & infinite(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & not there_is_enemy_at(V) & friend(_, Entity, explorer), Options)
						   ...
						   .nth(math.random(TotalOptions), Options, Op).
there_is_disabled_friend_nearby(Op) :- position(MyV) & myTeam(MyTeam) & maxWeight(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & not there_is_enemy_at(V) & friend(_, Entity, inspector), Options)
						   & .length(Options, TotalOptions) & TotalOptions > 0 &
						   .nth(math.random(TotalOptions), Options, Op).
there_is_disabled_friend_nearby(Op) :- position(MyV) & myTeam(MyTeam) & infinite(INF) & myNameInContest(MyName) &
						   .setof(V, ia.edge(MyV,V,W) & W \== INF & visibleEntity(Entity, V, MyTeam, disabled) & Entity \== MyName & not there_is_enemy_at(V) & friend(_, Entity, inspector), Options)
...
//If I'm disabled and there is a normal repairer at some adjacent vertex and there is not saboteur in my vertex and my friend is not busy, so I'm going to wait 
is_wait_repair_goal_different_noOthers(V) :- is_disabled & position(MyV) & myTeam(MyTeam) & 
						ia.edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, normal) & friend(_, Entity, repairer) & 
						(not there_is_disable_friend_at_same_vertex_as_repairer(V) & not there_is_enemy_at(MyV)).
//If I'm disabled and there is a disabled repairer at some adjacent vertex and my friend is not busy, so I'm going to wait if my friend has higher priority
is_wait_repair_goal_different_noOthers(V) :- is_disabled & position(MyV) & myTeam(MyTeam) & .my_name(MyAgentName) & 
						ia.edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, disabled) & friend(AgentName, Entity, repairer) & priorityEntity(AgentName, MyAgentName) & MyAgentName \== AgentName & 
						not there_is_disable_friend_at_same_vertex_as_repairer(V).
						
//If I'm disabled and there is a repairer at some adjacent vertex and my friend is busy or there is enemy in my vertex, I'm going there
is_wait_repair_goal_different(V) :- is_disabled & position(MyV) & myTeam(MyTeam) & 
						ia.edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, normal) & friend(_, Entity, repairer) & 
						(there_is_disable_friend_at_same_vertex_as_repairer(V) | there_is_enemy_at(MyV)).
is_wait_repair_goal_different(V) :- is_disabled & position(MyV) & myTeam(MyTeam) & 
						ia.edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, disabled) & friend(_, Entity, repairer) & 
						(there_is_disable_friend_at_same_vertex_as_repairer(V) | there_is_enemy_at(MyV)).
...
+!select_goal: //TODO ajustar para 3
	(there_is_more_repairers(RepairerNameFriend) & numberWaits(K) & K >= 14) | (step(0) & not nextStepRepairerAll(10))
<-
	.print("I can't wait anymore!");
	//-numberWaits(_);
	//+numberWaits(0);
	-+numberWaits(0);
	//-nextStepRepairerAll(_);
...
//+!select_goal : there_is_disabled_repairer_friend_nearby(Op) & there_is_enemy_at(Op) <- !init_goal(recharge). //Wait the friend
+!select_goal : there_is_disabled_repairer_friend_nearby(Op) <- !init_goal(followFriend(Op)).
+!select_goal : there_is_disabled_friend_nearby(Op) & not there_is_enemy_at(Op) <- !init_goal(followFriend(Op)).
...
+!select_goal : is_wait_repair_goal_different_noOthers(_) <-
	.print("Waiting to be repaired."); 
	!init_goal(recharge). //Instead of don't do anything, then recharge
+!select_goal : is_wait_repair_goal_different(V) <-
	.print("Waiting to be repaired. Going there"); 
	!init_goal(walkRepairByRepairer(V)).
...
+!select_goal : is_route_goal & not is_almost_full_map <- !init_goal(random_walk). //I'm going to cure some agent
...
/*
+!select_goal : waitingDecisionProposal(_)      <-
	.print("I'm waiting a decision of a proposal..."); //TODO dah pra fazer melhor, sem o reparador esperar, basta ele fazer oq ker e depois recalcular a rota. 
	!init_goal(recharge). //I'm waiting because I don't know if I have the best proposal
	*/
...
//@hit0[atomic] //TODO se colocar como atomic mensagens chegam depois que terminou de processar apenas
+hit(AgentName, Pos):
	infinite(INF)
<-
...
//VERSAO ANTIGA QUE ESPERA A DECISAO
/*
+!gotoCure(AgentName):
	pathProposal(AgentName, D, Path, Lenght)
<-
