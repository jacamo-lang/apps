...
is_wait_repair_goal_general(MyV) :- is_disabled & position(MyV) & myTeam(MyTeam) & myNameInContest(MyName) & 
...
is_wait_repair_goal_general(V) :- is_disabled & position(MyV) & myTeam(MyTeam) & myNameInContest(MyName) &
		       ia.edge(MyV,V,_) & visibleEntity(Entity, V, MyTeam, normal) & friend(_, Entity, repairer) & Entity \== MyName.
is_wait_repair_goal_general(V) :- is_disabled & position(MyV) & myTeam(MyTeam) & myNameInContest(MyName) &
