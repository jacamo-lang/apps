...
+!calcWaytogoTo([], Path, PosAim):
	true
<-
	.print("SWARM: I do not know any way to go to the best places!").

+!calcWaytogoTo([D|T], Path, PosAim):
	currentPosition(S)
<-
	.print("SWARM: Calculating path between ", S, " and ", D, " I also have other options ", T);
...
-!calcWaytogoTo([D|T], Path, PosAim):
	currentPosition(S)
<-
	.print("SWARM: I do not know a way between ", S, " and ", D, " so I will try the other options ", T);
	!calcWaytogoTo(T, Path, PosAim).
	
+!gotoSwarmPlace:
	bestProbedVertex(V, Value) & list_vertex_by_value(Value, List)
<-
	.print("SWARM: Going to some best vertex! ", List);
	!calcWaytogoTo(List, Path, PosAim);
