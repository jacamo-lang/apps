...
is_buy_goal    :-(
					...
					strength(MyS) & strengthRequired(Sr) & MyS < Sr & .my_name(saboteur1) 
					|
...
get_vertex_to_go_attack(D, Path) :- position(MyV) & myTeam(MyTeam) & myNameInContest(MyName) & 
											...
											.setof(V,
												visibleEntityTell(Entity, V, _, _) & 
...
//+!select_goal : not is_disabled & there_is_enemy_inside_my_good_area(V) <- !init_goal(walk_attack_area).
...
+!buy:
	money(M) & strength(MyS) & strengthRequired(Sr) & MyS < Sr & .my_name(saboteur1)
<- 
	.abolish(canBuy);
...
+!setEntityHealth(Entity, MaxHealth):
	strengthRequired(Current) & MaxHealth > Current & entityType(Entity, "Saboteur")
<-
