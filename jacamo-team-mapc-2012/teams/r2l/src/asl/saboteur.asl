...
is_buy_goal    :-(
					...
					strenght(MyS) & strengthRequired(Sr) & MyS < Sr //not buy
				 ) & money(MyM) & MyM > 2.
...
+!select_goal: //TODO ajustar para 3
	there_is_more_saboteurs(SaboteurNameFriend) & numberWaits(K) & K >= 14
<-
...
+!select_goal : is_buy_goal & 
				...
				not is_disabled & 
				position(Pos) & 
				not there_is_enemy_at(Pos) &
				strength(MyS) & MyS < 5  <- !init_goal(buy).
+!select_goal : 
				is_buy_goal & 
				not canBuy & 
				not is_disabled & 
