...
//exist_vertex_to_probe(Vertex) :-
							


get_best_coverage_proposal(BestVertex, BestValue, Neighborhood) :-
							bestCoverageProposal(_, BestValueProposal, _, _) &
							.setof(Value, bestCoverageProposal(_, Value, _, _), SetValues) &
							.max(SetValues, BestValue) &
							bestCoverageProposal(BestVertex, BestValue, Neighborhood, _).


/* The sequence of the priority plans */
/*
+!select_goal: 
	not there_is_possible_best_vertex & .my_name(explorer1) 
<- 
	!setSwarmMode; 
	!select_goal. */
...
+!select_goal: //TODO ajustar para 3
	(numberWaits(K) & K >= 14) | (step(0) & not nextStepExplorerAll(10))
<-
...
+!select_goal : is_leave_goal & not is_disabled <- .print("Enemy here, leaving..."); !init_goal(searchBestVertex).
...
+!select_goal : not noMoreVertexToProbe <- !init_goal(searchBestVertex).
...
+!select_goal : is_at_swarm_position_test <-
...
+!setSwarmMode:
	bestProbedVertex(V, Value)
<-
	.broadcast(tell, bestProbedVertex(V, Value));
	.print("## One possible best vertex was found! ", V, " ", Value).
	* 
	*/

+!newStepPosAction(S):
	.my_name(explorer1) & not ia.thereIsUnprobedVertex & not noMoreVertexToProbe
<-
	+noMoreVertexToProbe;
	!!tryProposeBestCoverage(S).
	
+!newStepPosAction(S): //for security
	.my_name(explorer1) & ia.thereIsUnprobedVertex & noMoreVertexToProbe
<-
	-noMoreVertexToProbe;
	!!tryProposeBestCoverage(S).
...
+!tryProposeBestCoverage(S):	
	lastCalcCoverage(N) & S - N >= 15 & .my_name(explorer1) & S < 100
<-
	.abolish(bestCoverageProposal(_, _, _, _)); 
	-secondPlace;
	.send(explorer2, achieve, tryProposeBestCoverage(S));
	.send(explorer3, achieve, tryProposeBestCoverage(S));
	.send(explorer4, achieve, tryProposeBestCoverage(S));
	-+lastCalcCoverage(S);
	...
	+bestCoverageProposal(BestVertex, BestValue, Neighborhood, explorer1);
	.print("COVERAGE Result was... BestVertex: ", BestVertex, " BestValue: ", BestValue, " Neighborhood: ", Neighborhood).
	
+!tryProposeBestCoverage(S):	
	lastCalcCoverage(N) & S - N >= 15 & .my_name(AgentName) & S < 100
<- 
	-+lastCalcCoverage(S);
	.print("COVERAGE Calculing the best coverage...");
	ia.bestCoverage(1, BestVertex, BestValue);
	ia.neighborhood(BestVertex, 2, Neighborhood);
	.send(explorer1, tell, bestCoverageProposal(BestVertex, BestValue, Neighborhood, AgentName));
	.print("COVERAGE Result was... BestVertex: ", BestVertex, " BestValue: ", BestValue, " Neighborhood: ", Neighborhood).
	 
+!tryProposeBestCoverage(S):	
	lastCalcCoverage(N) & S - N >= 15 & .my_name(explorer1)
<-
	.abolish(bestCoverageProposal(_, _, _, _)); 
	-secondPlace;
	.send(explorer2, achieve, tryProposeBestCoverage(S));
...
+!tryProposeBestCoverage(S):	
	lastCalcCoverage(N) & S - N >= 15 & .my_name(AgentName)
<- 
	-+lastCalcCoverage(S);
	.print("COVERAGE Calculing the best coverage...");
	ia.bestCoverage(2, BestVertex, BestValue);
...
+!tryProposeBestCoverageTwo(S, BestVertexCurrent):	
	.my_name(explorer1) & S < 100
<-
	.abolish(bestCoverageProposal(_, _, _, _)); 
	+secondPlace;
	.send(explorer2, achieve, tryProposeBestCoverageTwo(S, BestVertexCurrent));
	.send(explorer3, achieve, tryProposeBestCoverageTwo(S, BestVertexCurrent));
	.send(explorer4, achieve, tryProposeBestCoverageTwo(S, BestVertexCurrent));
...
+bestCoverageProposal(_, _, _, _):
	.count(bestCoverageProposal(_, _, _, _), N) & N >= 4 & secondPlace
<-
	.print("COVERAGE ALL TWO Proposal arrived! ", N);
	!decideBestCoverageProposalTwo.
	
+bestCoverageProposal(_, _, _, _):
	.count(bestCoverageProposal(_, _, _, _), N) & N >= 4
<-
	.print("COVERAGE ALL Proposal arrived! ", N);
	!decideBestCoverageProposal.
	
+!decideBestCoverageProposalTwo: //the second good place
	get_best_coverage_proposal(BestVertex, BestValue, Neighborhood)
<-
	.abolish(bestCoverageProposal(_, _, _, _));
	-secondPlace;
	.send(explorer3, achieve, setNewBestCoverageSwarm(BestVertex, BestValue, Neighborhood));
...
+!decideBestCoverageProposal:
	get_best_coverage_proposal(BestVertex, BestValue, Neighborhood) & step(S)
<-
	.abolish(bestCoverageProposal(_, _, _, _));
