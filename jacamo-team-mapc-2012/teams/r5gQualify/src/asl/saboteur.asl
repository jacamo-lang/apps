...
is_buy_attack_goal    :- 
	strength(MyS) &
	(MyS < 4 | strengthRequired(Sr) & MyS < Sr).
...
+!select_goal: //TODO ajustar para 3
	(there_is_more_saboteurs(SaboteurNameFriend) & numberWaits(K) & K >= 4) | (step(0) & not nextStepSaboteurAll(10))
<-
...
+!select_goal: .my_name(saboteur4) & noMoreVertexToProbe & not is_at_swarm_position & get_vertex_to_go_swarm(D, Path) <-
	!clearGettingAround;
...
<-
	//.print("Entity ", Entity, " is not my friend!");
	!evaluateEntityNewMaxStrength(Entity);
	!evaluateEntityHealth(Entity).
